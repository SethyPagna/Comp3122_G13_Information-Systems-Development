  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      if (error.message?.toLowerCase().includes('email not confirmed')) {
        toast.error('Please confirm your email first, or disable email confirmation in Supabase Dashboard → Auth → Settings.')
  ...
  return (
    <div className="atlas-card p-8">
      <form onSubmit={handleLogin} className="space-y-5">
        <div>
          ...
          <input type="email" value={email} onChange={e => setEmail(e.target.value)}
            placeholder="you@school.edu" required autoComplete="email" className="atlas-input" />
        </div>
        <div>
          <label className="block text-sm font-medium text-atlas-subtle mb-2">Password</label>
          <input type="password" value={password} onChange={e => setPassword(e.target.value)}
            placeholder="••••••••" required autoComplete="current-password" className="atlas-input" />
        </div>
      ...
      <p className="text-center text-atlas-subtle text-sm mt-6">
        ...
        <Link href="/auth/signup" className="text-atlas-blue hover:underline font-medium">Create one</Link>
      </p>
    </div>
  )
}
    ...
    <div className="min-h-screen bg-atlas-bg flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md animate-slide-up">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display font-bold text-2xl text-atlas-text">Welcome back</h1>
